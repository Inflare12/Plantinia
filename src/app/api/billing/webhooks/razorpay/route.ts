...
export async function POST(req: NextRequest) {
  try {
    ...
    if (event.event === 'order.paid' || event.event === 'payment.captured') {
      const paymentEntity = event.payload?.payment?.entity || {}; const notes = paymentEntity.notes || {};
      const userId = notes.userId; const tierValue = notes.tier;
      ...
      const paymentRef = String(paymentEntity.id || ''); if (!paymentRef) return NextResponse.json({ error: 'Missing Razorpay payment ID' }, { status: 400 });

      let duplicate = false;
      try { await db.invoices.create({ userId: user.id, amount: plan.priceINR, currency: 'INR', provider: 'razorpay', providerPaymentId: paymentRef, status: 'paid', plan: plan.name, receiptUrl: `#receipt-${paymentRef}` }); }
      ...
      const updated = await db.users.update(user.id, { subscriptionTier: plan.id, subscriptionStatus: 'active', subscriptionCurrentPeriodEnd: new Date(Date.now() + 30 * 24 * 3600 * 1000).toISOString(), paymentProvider: 'razorpay', subscriptionId: paymentEntity.order_id || paymentEntity.id, creditsRemaining: plan.creditsPerMonth === 'unlimited' ? 9999 : plan.creditsPerMonth });
  ...
  } catch (error) { console.error('Razorpay webhook error:', error); return NextResponse.json({ error: 'Webhook processing failed' }, { status: 400 }); }
}
