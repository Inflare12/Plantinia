...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try { const user=await getSessionUser(req); if(!user)return NextResponse.json({error:'Unauthorized'},{status:401}); const plant=await db.plants.findById(params.id); if(!plant||(plant.userId!==user.id&&user.role!=='admin'))return NextResponse.json({error:'Plant not found'},{status:404}); const [diagnoses,timeline]=await Promise.all([db.diagnoses.listByPlant(plant.id),db.timeline.listByPlant(plant.id)]); return NextResponse.json({plant,diagnoses,timeline}); }
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try { const user=await getSessionUser(req); if(!user)return NextResponse.json({error:'Unauthorized'},{status:401}); const plant=await db.plants.findById(params.id); if(!plant||(plant.userId!==user.id&&user.role!=='admin'))return NextResponse.json({error:'Plant not found'},{status:404}); const parsed=updatePlantSchema.safeParse(await req.json()); if(!parsed.success)return NextResponse.json({error:'Invalid plant update',details:parsed.error.flatten()},{status:400}); const data={...parsed.data,lastWateredDate:parsed.data.lastWateredDate===null?undefined:parsed.data.lastWateredDate,nextWateringDate:parsed.data.nextWateringDate===null?undefined:parsed.data.nextWateringDate}; const updated=await db.plants.update(params.id,data); return NextResponse.json({plant:updated}); }
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try { const user=await getSessionUser(req); if(!user)return NextResponse.json({error:'Unauthorized'},{status:401}); const plant=await db.plants.findById(params.id); if(!plant||(plant.userId!==user.id&&user.role!=='admin'))return NextResponse.json({error:'Plant not found'},{status:404}); await db.plants.delete(params.id); return NextResponse.json({success:true}); }
