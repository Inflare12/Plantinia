...
const updatePlantSchema = z.object({
  ...
  healthStatus: z.enum(['healthy', 'warning', 'critical', 'treating']).optional(),
  sunlightNeeds: z.enum(['direct', 'indirect', 'low', 'shade']).optional(),
  wateringFrequencyDays: z.coerce.number().int().min(1).max(365).optional(),
  lastWateredDate: z.string().datetime().optional().nullable(),
  nextWateringDate: z.string().datetime().optional().nullable(),
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ error: 'Unable to load plant' }, { status: 500 });
  }
}
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const updated = await db.plants.update(params.id, parsed.data);
    return NextResponse.json({ plant: updated });
  } catch (error) {
    console.error('Plant PATCH error:', error);
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ error: 'Unable to delete plant' }, { status: 500 });
  }
}
